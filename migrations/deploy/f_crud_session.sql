...
RETURNS TABLE (id INTEGER, name TEXT, sequence_id INTEGER, card_id INTEGER, activity_id INTEGER, comments TEXT, "time" INTEGER, is_face_to_face BOOLEAN, is_group_work BOOLEAN, equipment TEXT) AS
$$
DECLARE 
    data JSONB := json_data::JSONB;
BEGIN
    ...
    SET
        ...
        activity_id = COALESCE(CAST(data->>'activity_id' AS INTEGER), session.activity_id),
    ...
    RETURNING session.id, session.name, session.sequence_id, session.card_id, session.activity_id, session.comments, session."time", session.is_face_to_face, session.is_group_work, session.equipment;
END;
$$ LANGUAGE plpgsql;
